    ...
    <el-form ref="form" :model="form" label-width="100px">
      <el-form-item label="查询关键字">
        <el-select v-model="form.keyword" placeholder="请选择关键字" @change="keywordChange">
          <el-option label="SECTOR_ID" value="SECTOR_ID" />
          <el-option label="SECTOR_NAME" value="SECTOR_NAME" />
        </el-select>
      </el-form-item>
...
import { getSectorId, getSectorName } from '@/api/query'
...
export default {
  ...
  methods: {
    onSubmit() {
      ...
      this.$store.dispatch('query_operate/universeQuery', data).then(res => {
        ...
        if (res.detail.total_pages === 0) {
          ...
          for (var key in this.$data.listResult[0]) {
    ...
    keywordChange() {
      ...
      if (this.form.keyword === 'SECTOR_ID') {
        getSectorId().then(res => {
